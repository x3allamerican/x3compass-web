...
export const onRequestGet: PagesFunction<Env> = async (ctx) => {
  ...
  if (carrier_id) q += `&carrier_id=eq.${carrier_id}`;
  if (status && status !== "all") q += `&status=eq.${status}`;
  if (platform && platform !== "all") q += `&platform=eq.${platform}`;
  ...
  const countsR = await fetch(`${ctx.env.SUPABASE_URL}/rest/v1/compass_social_posts?select=status,platform${carrier_id ? `&carrier_id=eq.${carrier_id}` : ""}&limit=10000`, { headers: { apikey: ctx.env.SUPABASE_SERVICE_ROLE, Authorization: `Bearer ${ctx.env.SUPABASE_SERVICE_ROLE}`, Accept: "application/json" } });
