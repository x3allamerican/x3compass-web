/** POST /api/vendors/trimble/sync — pull Trimble HOS daily-logs into compass_hos_logs (§395.3-scored). Body: { carrier_id }. */
import { correlationId, requireTenant, securityError, type SecurityEnv } from "../../../_shared/request-security";
import { syncEldVendor, trimbleConfig, type EldVendorEnv } from "../../../_shared/eld-hos-sync";
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  const cfg = trimbleConfig(ctx.env);
  if (!cfg) return json({ ok: false, configured: false, vendor: "trimble_connect", error: "Trimble not configured. Set TRIMBLE_API_KEY on Cloudflare Pages and redeploy.", help_url: "https://www.trimbleconnect.com/integrations/" }, 503);
