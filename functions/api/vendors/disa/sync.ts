/** POST /api/vendors/disa/sync — pull DISA screening results into vendor_orders. Body: { carrier_id }. */
import { correlationId, requireTenant, securityError, type SecurityEnv } from "../../../_shared/request-security";
import { syncScreeningVendor, disaConfig, type ScreeningVendorEnv } from "../../../_shared/screening-sync";
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  const cfg = disaConfig(ctx.env);
  if (!cfg) return json({ ok: false, configured: false, vendor: "disa", error: "DISA not configured. Set DISA_API_KEY on Cloudflare Pages and redeploy.", help_url: "https://developer.disa.com" }, 503);
