/** POST /api/vendors/sambasafety/sync — pull SambaSafety screening results into vendor_orders. Body: { carrier_id }. */
import { correlationId, requireTenant, securityError, type SecurityEnv } from "../../../_shared/request-security";
import { syncScreeningVendor, sambaConfig, type ScreeningVendorEnv } from "../../../_shared/screening-sync";
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  const cfg = sambaConfig(ctx.env);
  if (!cfg) return json({ ok: false, configured: false, vendor: "sambasafety", error: "SambaSafety not configured. Set SAMBASAFETY_API_KEY on Cloudflare Pages and redeploy.", help_url: "https://developer.sambasafety.com" }, 503);
