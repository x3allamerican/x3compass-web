/** POST /api/vendors/geotab/sync — pull Geotab HOS daily-logs into compass_hos_logs (§395.3-scored). Body: { carrier_id }. */
import { correlationId, requireTenant, securityError, type SecurityEnv } from "../../../_shared/request-security";
import { syncEldVendor, geotabConfig, type EldVendorEnv } from "../../../_shared/eld-hos-sync";
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  const cfg = geotabConfig(ctx.env);
  if (!cfg) return json({ ok: false, configured: false, vendor: "geotab_connect", error: "Geotab not configured. Set GEOTAB_API_KEY on Cloudflare Pages and redeploy.", help_url: "https://www.geotabconnect.com/integrations/" }, 503);
