...
// Skill cards on the homepage. Click opens SkillConciergeModal in-place —
// the live /api/ask + 300-skill corpus answers right there as an overlay,
// no page jump, no /signup wall.

import { useState } from "react";
import SkillConciergeModal from "./SkillConciergeModal";
...
// 100waystosay-style gradient stripes for skill cards · matches BrainGrid.
// 6 variants cycled by card index so the top edge has color rotation.
const SKILL_STRIPES = [
...
function SkillCard({ skill, onClick, i = 0 }: { skill: Skill; onClick: () => void; i?: number }) {
  const stripe = SKILL_STRIPES[i % SKILL_STRIPES.length];
  return (
    <button
      onClick={onClick}
      className="text-left rounded-2xl p-6 pt-7 border border-[#1E3556] bg-black hover:border-[#16C7FF]/50 hover:shadow-[0_0_28px_rgba(22,199,255,0.25)] hover:-translate-y-0.5 transition-all relative pr-10 group overflow-hidden"
    >
      {/* Top gradient stripe · matches BrainGrid */}
      <span
        aria-hidden="true"
        className="absolute left-0 right-0 top-0 h-[3px]"
        style={{ background: stripe }}
      />
      <div className="inline-block text-[10px] font-bold tracking-wider text-[#16C7FF] bg-[#16C7FF]/10 border border-[#16C7FF]/25 px-2 py-1 rounded-full font-mono mb-2">
        {skill.cfr}
      </div>
      ...
      <div className="absolute right-5 top-7 text-[#16C7FF] font-bold group-hover:translate-x-0.5 transition-transform">→</div>
...
export default function SkillsExplorer() {
  // Active skill drives the in-place modal. null = closed.
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {SKILLS.map((s, i) => (
          <SkillCard key={i} i={i} skill={s} onClick={() => setActive(s)} />
        ))}
      </div>
      <SkillConciergeModal
        question={active?.q ?? null}
        cfr={active?.cfr}
        name={active?.name}
