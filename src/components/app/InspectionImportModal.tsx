...
2026-03-08,Maria,Reyes,7XYZ124,2,OK,Officer 1184,INS-238311,false,true,4,`;
...
export function InspectionImportModal({ carrierId, onClose, onImported }: { carrierId: string; onClose: () => void; onImported: () => void }) {
  ...
  function downloadTemplate() {
    const a = document.createElement("a"); a.href = "/templates/inspections-import.csv"; a.download = "inspections-import.csv"; a.click();
  }
  ...
  async function submit() {
    ...
    try {
      ...
      const r = await fetch("/api/inspections/import", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ carrier_id: carrierId, csv }) });
      ...
      <div className="bg-[var(--surface-2)] rounded-2xl border border-[var(--border)] max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-5">
          <div>
            ...
            <button onClick={downloadTemplate} className="px-3 py-1.5 rounded-lg text-[12px] font-bold border border-[var(--border)] text-[var(--fg)] hover:bg-[var(--surface-3)]">↓ inspections_template.csv</button>
            <div className="text-[11px] text-[var(--fg-muted)] mt-1">Required: inspection_date + level (1-6). Driver matched by name; vehicle by plate.</div>
          </div>
          <div>
