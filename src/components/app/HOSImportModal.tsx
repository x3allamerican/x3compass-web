...
type ImportResult = { ok: boolean; submitted: number; inserted: number; updated: number; skipped: number; flagged?: number; errors: { row: number; reason: string }[] };

export function HOSImportModal({ carrierId, onClose, onImported }: { carrierId: string; onClose: () => void; onImported: () => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      const r = await fetch("/api/hos/import", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ carrier_id: carrierId, csv }) });
      const b = await r.json() as ImportResult & { error?: string };
      if (!r.ok) setError(b.error || `Server error ${r.status}`); else { setResult(b); if (b.inserted > 0) onImported(); }
    } catch (err) { setError(err instanceof Error ? err.message : String(err)); } finally { setBusy(false); }
  }

  return (
    <div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="hos-modal-title">
      <div className="bg-[var(--surface-2)] rounded-2xl border border-[var(--border)] max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="p-6 border-b border-[var(--border)] flex items-center justify-between">
          <div>
            <h2 id="hos-modal-title" className="text-[20px] font-extrabold text-[var(--fg)]">Import HOS daily logs</h2>
            <p className="text-[12px] text-[var(--fg-muted)] mt-1">49 CFR § 395.3 · X3 computes 11-hr / 14-hr / 30-min / 70-8 violations for you.</p>
          </div>
        ...
        <div className="p-6 space-y-5">
          <div>
            <div className="text-[13px] font-bold text-[var(--fg)] mb-1">1. Download the template</div>
            <a href="/templates/hos-log-import.csv" download className="inline-block px-3 py-1.5 rounded-lg text-[12px] font-bold border border-[var(--border)] text-[var(--fg)] hover:bg-[var(--surface-3)]">↓ hos-log-import.csv</a>
            <div className="text-[11px] text-[var(--fg-muted)] mt-1">Columns: driver_id (or driver_first_name + driver_last_name), log_date, drive_min, on_duty_min, hours_70_8, cycle, break_min, eld_source, certified.</div>
          </div>
          <div>
          ...
          {result && (
            <div className={`rounded-lg border p-4 text-[13px] ${result.ok ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "border-amber-500/40 bg-amber-500/10 text-amber-700 dark:text-amber-300"}`}>
              ...
              <div className="text-[12px]">Saved: <strong>{result.inserted}</strong> logs{typeof result.flagged === "number" ? <> · <strong>{result.flagged}</strong> with HOS violations</> : null}{result.skipped ? <> · Skipped: <strong>{result.skipped}</strong></> : null}</div>
              {result.errors.length > 0 && <ul className="mt-2 list-disc pl-5 text-[11px]">{result.errors.slice(0, 20).map((item) => <li key={`${item.row}-${item.reason}`}>CSV row {item.row}: {item.reason}</li>)}</ul>}
            </div>
          )}
        </div>
