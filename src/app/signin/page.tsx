  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--fg)] grid place-items-center px-6 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="rounded-2xl p-9 border border-[var(--border)]" style={{ background: "linear-gradient(180deg, var(--surface) 0%, var(--surface-3) 100%)", boxShadow: "0 24px 60px rgba(0, 0, 0, 0.45)" }}>
          ...
          <div className="flex gap-1 mb-6 p-1 rounded-lg bg-[var(--bg)] border border-[var(--border)]">
            <button className="flex-1 py-2.5 rounded-md font-bold text-[13px] text-[var(--bg)]" style={{ background: "linear-gradient(135deg, var(--accent), var(--accent-2))" }}>Sign In</button>
            ...
            <>
              <form onSubmit={mode === "password" ? handlePasswordSubmit : handleMagicSubmit} className="space-y-4">
                <div>
                  ...
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@yourfleet.com" className="w-full px-4 py-3 rounded-lg bg-[var(--bg)] border border-[var(--border)] text-[var(--fg)] text-[14px] focus:outline-none focus:border-[var(--accent)]" />
                </div>
                {mode === "password" && (
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <label className="text-[11px] tracking-[.14em] uppercase text-[var(--fg-muted)] font-bold">Password</label>
                      <Link href="/forgot-password" className="text-[11px] text-[var(--accent)] hover:underline">Forgot?</Link>
                    </div>
                    <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full px-4 py-3 rounded-lg bg-[var(--bg)] border border-[var(--border)] text-[var(--fg)] text-[14px] focus:outline-none focus:border-[var(--accent)]" />
                  </div>
                )}
              ...
              <div className="mt-6 text-center">
                <button onClick={() => setMode(mode === "password" ? "magic" : "password")} className="text-[12px] text-[var(--fg-muted)] hover:text-[var(--accent)]">
                  {mode === "password" ? "Or sign in with a magic link →" : "← Back to password sign-in"}
                </button>
              </div>
              <div className="mt-8 pt-6 border-t border-[var(--border)] text-center">
                <p className="text-[12px] text-[var(--fg-muted)]">New to X3 Compass? <Link href="/signup" className="text-[var(--accent)] font-bold">Start your free trial →</Link></p>
              </div>
            </>
          )}
        </div>
