...
function BillingInner() {
  ...
  if (loading) return <div className="min-h-screen bg-[#0A1929] grid place-items-center text-white/55">Loading…</div>;
  if (!carrier) return <div className="min-h-screen bg-[#0A1929] grid place-items-center text-white/55">No carrier linked yet — refresh.</div>;
  ...
  return (
    <div className="min-h-screen bg-[#0A1929] text-white">
      <div className="max-w-3xl mx-auto px-6 py-12">
        <Link href="/app/settings" className="text-[12px] text-white/55 hover:text-white inline-flex items-center gap-2 mb-6">← Back to settings</Link>
        <h1 className="text-3xl font-extrabold mb-2">Billing</h1>
        <p className="text-white/55 mb-8">Manage subscription, payment method, invoices, and tax info.</p>
        ...
        <div className="rounded-2xl p-6 mb-6 bg-[#0F1C32] border border-[#1E3556]">
          <div className="text-[11px] tracking-[.18em] uppercase text-white/55 font-bold mb-3">Plan</div>
          <div className="flex items-center justify-between mb-1">
            <div className="text-xl font-extrabold">{(carrier.service_tier || "diy").toUpperCase()} {carrier.hazmat_addon ? "+ Hazmat" : ""}</div>
            <div className={`text-[12px] font-bold uppercase tracking-[.14em] ${isActive ? "text-green-400" : isTrial ? "text-[#22D3EE]" : isPastDue ? "text-orange-400" : "text-white/55"}`}>{carrier.subscription_status}</div>
          </div>
          {isTrial && carrier.trial_ends_at && <div className="text-[12px] text-white/55">Trial ends {new Date(carrier.trial_ends_at).toLocaleDateString()}</div>}
        </div>
        <div className="grid md:grid-cols-2 gap-4">
          <button onClick={openPortal} disabled={portalLoading} className="rounded-2xl p-6 bg-[#0F1C32] border border-[#1E3556] hover:border-[#22D3EE] text-left">
            <div className="text-xl font-extrabold mb-1">Manage subscription</div>
            <div className="text-[12px] text-white/55">Update card · cancel · invoices · change plan</div>
            {portalLoading && <div className="text-[11px] text-[#22D3EE] mt-3">Opening Stripe portal…</div>}
          </button>
          <Link href="/pricing" className="rounded-2xl p-6 bg-[#0F1C32] border border-[#1E3556] hover:border-[#22D3EE] block">
            <div className="text-xl font-extrabold mb-1">Switch plan</div>
            <div className="text-[12px] text-white/55">Compare DIY vs DFY vs Enterprise</div>
          </Link>
        </div>
...
export default function BillingPage() {
  return (<Suspense fallback={<div className="min-h-screen bg-[#0A1929]" />}><BillingInner /></Suspense>);
}
