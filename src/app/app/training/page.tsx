...
export default function TrainingPage() {
  ...
  return (
    <AppShell crumbs="TRAINING" title="Training Records"
      actions={<button onClick={() => setShowAdd(true)} disabled={!drivers.length} className="px-4 py-2 rounded-lg font-extrabold text-[12px] text-[#0A1929] disabled:opacity-50" style={{ background: "linear-gradient(135deg, #22D3EE, #06B6D4)" }}>+ Add training</button>}>
      <div className="p-6">
        <TenantTable<T> rows={rows} loading={loading}
          ...
          columns={[
            { key: "driver", label: "Driver", render: (r) => <span className="text-white">{driverLabel(drivers.find(d => d.id === r.driver_id))}</span> },
            { key: "course_name", label: "Course", render: (r) => <span className="text-white">{r.course_name}</span> },
            ...
            { key: "expires_on", label: "Expires", render: (r) => !r.expires_on ? <span className="text-white/35">—</span> : r.expires_on < today ? <Badge color="red">{fmtDate(r.expires_on)}</Badge> : <span className="text-white/85">{fmtDate(r.expires_on)}</span> },
