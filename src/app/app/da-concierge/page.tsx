    ...
    >
      <div className="px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {[
            { l: "Random pool size",           v: "72 CDL", c: "#22D3EE" },
        ...
        <div className="rounded-2xl border border-[#1E3556] overflow-hidden" style={{ background: "linear-gradient(180deg, #15233D 0%, #0F1C32 100%)" }}>
          ...
          <div className="divide-y divide-[#1E3556]">
            {SERVICE_ITEMS.map((s, i) => {
              ...
              return (
                <div key={i} className="px-5 py-4 flex items-center gap-4 hover:bg-[#22D3EE]/5">
                  ...
                  <button className={`text-[13px] font-bold whitespace-nowrap ${isAction ? "text-[#0A1929] px-4 py-2 rounded-full" : "text-[#22D3EE] hover:text-[#67E8F9]"}`}
                    style={isAction ? { background: "linear-gradient(135deg, #22D3EE, #06B6D4)" } : undefined}
                  >
                    {s.action} →
                  </button>
                </div>
            ...
            {ADVISOR_TIMELINE.map((t, i) => (
              <div key={i} className="px-5 py-4 flex gap-4 items-start">
                <div className={`w-9 h-9 rounded-full grid place-items-center font-bold text-[11px] flex-shrink-0 ${
                  ...
                  style={t.actor === "Compass" ? { background: "linear-gradient(135deg, #22D3EE, #06B6D4)" } : undefined}
                >
