      ...
      <div className="p-6 max-w-3xl space-y-6">
        <div className="rounded-2xl border border-[#22D3EE]/50 p-6" style={{ background: "linear-gradient(180deg, #15233D, #0F1C32)" }}>
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/app/drug-alcohol" className="px-5 py-2.5 rounded-lg font-extrabold text-[13px] text-[#0A1929]" style={{ background: "linear-gradient(135deg, #22D3EE, #06B6D4)" }}>View test history →</Link>
            <a href="mailto:joshua@x3compass.com?subject=Enroll%20in%20D%26A%20Concierge" className="px-5 py-2.5 rounded-lg font-bold text-[13px] text-white border border-[#1E3556] hover:border-[#22D3EE]">Email to enroll</a>
          </div>
        </div>
        <p className="text-[12px] text-white/55">On the DIY plan? You can self-manage from the <Link href="/app/drug-alcohol" className="text-[#22D3EE] underline">Drug & Alcohol</Link> tracker. Upgrade to DFY anytime from <Link href="/app/settings/billing" className="text-[#22D3EE] underline">Billing</Link>.</p>
      </div>
    </AppShell>
  );
}
...
function Bullet({ children }: { children: React.ReactNode }) {
  return <div className="flex items-start gap-2 text-[13px] text-white/85"><span className="text-[#22D3EE] mt-0.5">✓</span><span>{children}</span></div>;
}
