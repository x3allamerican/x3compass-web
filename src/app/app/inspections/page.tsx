...
export default function InspectionsPage() {
  ...
  return (
    <AppShell crumbs="INSPECTIONS" title="Roadside Inspections"
      actions={<button onClick={() => setShowAdd(true)} className="px-4 py-2 rounded-lg font-extrabold text-[12px] text-[var(--bg)]" style={{ background: "linear-gradient(135deg, var(--accent), var(--accent-2))" }}>+ Log inspection</button>}>
      <div className="p-6">
        ...
        <TenantTable<I> rows={rows} loading={loading}
          ...
          columns={[
            ...
            { key: "level", label: "Level", render: (i) => i.level ? <Badge color="cyan">Lvl {i.level}</Badge> : <span className="text-[var(--fg-faint)]">—</span> },
            { key: "state", label: "State", hideOnMobile: true, render: (i) => i.state || <span className="text-[var(--fg-faint)]">—</span> },
            { key: "driver_id", label: "Driver", hideOnMobile: true, render: (i) => i.driver_id ? driverLabel(drivers.find(d => d.id === i.driver_id)) : <span className="text-[var(--fg-faint)]">—</span> },
            ...
            { key: "report_url", label: "Report", render: (i) => i.report_url ? <a href={i.report_url} target="_blank" rel="noopener noreferrer" className="text-[var(--accent)] underline">View</a> : <span className="text-[var(--fg-faint)]">—</span> },
          ]}
        />
      </div>
