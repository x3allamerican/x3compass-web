...
function renderMd(s: string): string {
  return s
    ...
    .replace(/\*(.+?)\*/g, "<em>$1</em>")
    .replace(/`([^`]+)`/g, '<code class="text-[var(--accent)] bg-[var(--bg)] px-1 rounded">$1</code>')
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-[var(--accent)] underline" target="_blank" rel="noopener noreferrer">$1</a>')
    .replace(/\n/g, "<br/>");
}
          ...
          <div className="space-y-4 mb-6">
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${m.role === "user" ? "justify-end" : ""}`}>
                {m.role === "assistant" && <div className="w-8 h-8 rounded-full grid place-items-center font-black text-sm flex-shrink-0 text-[var(--bg)]" style={{ background: "linear-gradient(135deg, var(--accent), var(--accent-2))" }}>∞</div>}
                <div className={`rounded-2xl px-4 py-3 max-w-[80%] ${m.role === "user" ? "bg-[var(--accent)]/15 border border-[var(--accent)]/30 text-[var(--fg)]" : "bg-[var(--surface-3)] border border-[var(--border)] text-[var(--fg)]"}`}>
