...
/* Tiny client island — just the live UN→placard demo.
   Kept narrow on purpose so the parent /hazmat/page.tsx
  ...
  useEffect(() => {
    ...
    const render = () => {
      if (typeof window.renderPlacardSvg === "function") {
        ...
        wrap.innerHTML = `<div style="width:200px;height:200px;background:rgba(22, 199, 255,0.1);border:2px solid #16C7FF;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#16C7FF;font-weight:800;">UN ${un}</div>`;
      }
    };
    render();
