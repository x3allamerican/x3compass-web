...
function EnrollModal({ drivers, initialDriverId, onClose, onEnrolled }: {
  ...
  async function handleEnroll(e: FormEvent) {
    ...
    try {
      ...
      if (!j.ok) {
        if (j.code === "NEEDS_BASELINE") setError("This driver needs a completed baseline Checkr MVR first. Order one from /background-checks, then come back once it returns 'clear'.");
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="p-10 text-center">
                ...
                <p className="text-[12px] text-[var(--fg-muted)]">Add drivers in /drivers · MVRs attach to drivers.</p>
              </div>
            ) : (
