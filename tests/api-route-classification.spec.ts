...
test("tenant and privileged routes are never public", () => {
  for (const path of [
    "/api/scorecards", "/api/compliance-calendar", "/api/dashboard", "/api/notifications", "/api/drivers/import",
...
test("every caller-selectable tenant route invokes the shared membership guard", () => {
  const routes = [
    "accidents/import.ts", "compliance-calendar.ts", "dashboard.ts", "drivers/import.ts", "inspections/import.ts",
