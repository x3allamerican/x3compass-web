...
test("tenant and privileged routes are never public", () => {
  for (const path of [
    "/api/scorecards", "/api/accident-register", "/api/audit/pdf", "/api/compliance-calendar", "/api/dashboard", "/api/dataq/challenges", "/api/notifications", "/api/drivers/import",
...
test("every caller-selectable tenant route invokes the shared membership guard", () => {
  const routes = [
    "accident-register.ts", "accidents/import.ts", "audit/pdf.ts", "compliance-calendar.ts", "dashboard.ts", "dataq/challenges.ts", "drivers/import.ts", "inspections/import.ts",
