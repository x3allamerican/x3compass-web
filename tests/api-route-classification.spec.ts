...
test("every caller-selectable tenant route invokes the shared membership guard", () => {
  const routes = [
    "accident-register.ts", "accidents/import.ts", "audit/pdf.ts", "clearinghouse/status.ts", "compliance-calendar.ts", "dashboard.ts", "dataq/challenges.ts", "drivers/import.ts", "inspections/import.ts",
