...
function renderPlacardSvg(hazardClass, unNumber, size) {
  ...
  if (file) {
    ...
    return `<div role="img" aria-label="${accLabel}" style="position:relative;display:inline-block;width:${size}px;height:${size}px;">
      <img src="/hazmat/placards/${file}" alt="" aria-hidden="true" style="width:${size}px;height:${size}px;display:block;" />
      ${unBadge}
    </div>`;
  }
...
function renderSpecialtyPlacard(key, size) {
  ...
  return `<img src="/hazmat/placards/${cfg.file}" alt="${cfg.name}" title="${cfg.name} — 49 CFR ${cfg.cfr}" style="width:${size}px;height:${size}px;display:block;" />`;
}
